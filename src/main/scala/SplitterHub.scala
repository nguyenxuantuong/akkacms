...
object SplitterHub {
  trait Splitter[T] {
    def source(predicate: T => Boolean): Source[T, NotUsed]
  }

  def sink[T]: Sink[T, Splitter[T]] = sink(bufferSize = 256)
  def sink[T](bufferSize: Int): Sink[T, Splitter[T]] = Sink.fromGraph(new SplitterHub[T](bufferSize))
}

class SplitterHub[T](bufferSize: Int) extends GraphStageWithMaterializedValue[SinkShape[T], SplitterHub.Splitter[T]] {
  ...
  val in: Inlet[T] = Inlet("SplitterHub.in")
  ...
  private final case class Consumer(id: Long, callback: AsyncCallback[ConsumerEvent], predicate: T => Boolean)
  ...
  override def createLogicAndMaterializedValue(inheritedAttributes: Attributes): (GraphStageLogic, SplitterHub.Splitter[T]) = {
    val idCounter = new AtomicLong()

    val logic = new BroadcastSinkLogic(shape)

    class SplitterSource extends SplitterHub.Splitter[T] {
      private def createNewSource(predicate: T => Boolean): GraphStage[SourceShape[T]] = {
        new GraphStage[SourceShape[T]] {
          val out: Outlet[T] = Outlet("SplitterHub.out")
              ...
              @tailrec def register(): Unit = {
                logic.state.get() match {
                  ...
                  case previousState @ Open(callbackFuture, registrations) ⇒
                    val newRegistrations = Consumer(id, callback, predicate) :: registrations
              ...
              if (offsetInitialized && (hubCallback ne null)) {
                ...
                elem match {
                  ...
                  case _ ⇒
                    ...
                    //only push to output port if elem matches the predicate
                    if(predicate(elem.asInstanceOf[T])) {
      ...
      def source(predicate: T => Boolean): Source[T, NotUsed] = {
        Source.fromGraph(createNewSource(predicate))
      }
    }

    (logic, new SplitterSource())
  }
}